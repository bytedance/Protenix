...
```json
{
  "proteinChain": {
    ...
    "templatesPath": "/path/to/template.cif",
    "templateChainId": "A"
  },
}
```
...
* `templatesPath`: The path to a precomputed or directly supplied template file. Supported formats include `.a3m` (e.g., generated by `hmmsearch`), `.hhr`, embedded template `.json`, and local `.cif`/`.mmcif` structure files. **Absolute paths are recommended.**
  ...
  * For `.cif`/`.mmcif`, the structure is used directly as one template after sequence alignment to the input protein chain.
* `templateChainId`: Optional chain ID to use from a `.cif`/`.mmcif` template file. This is required when the template file contains multiple protein chains. It is ignored for `.a3m`, `.hhr`, and embedded template `.json` files.
...
> ⚠️ **Note**: When `templatesPath` is provided, the file must exist. Protenix treats explicit template paths as intentional user input and will not silently replace a missing file with automatic template-search results.
